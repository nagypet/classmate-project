...
{
  private readonly _token$ = new BehaviorSubject<SpvitaminSecurity.AuthorizationToken | null>(null);
  public readonly token$: Observable<SpvitaminSecurity.AuthorizationToken | null> = this._token$.asObservable();
  private refreshing = false;
  private refreshQueue$ = new Subject<void>();
  private refreshTimerSub: any;

  private _displayName$ = this._token$.pipe(map(t => t?.preferred_username ?? t?.sub));
  ...
  {
    return this._token$.pipe(map(t => t !== null));
  }
